...
# 2. Setup folders
$modDir = "c:\Users\meast\Downloads\Development\GAMES\rimworldModding\BlightedAlert"
...
# 4. Compile BlightedAlert.dll
Write-Host "Compiling BlightedAlert..." -ForegroundColor Green
...
$outputDll = "$assembliesDir\BlightedAlert.dll"
...
Write-Host "Successfully compiled BlightedAlert.dll!" -ForegroundColor Green

# 5. Deploy mod to RimWorld Mods folder
$deployDir = "$GamePath\Mods\BlightedAlert"
...
# Copy assemblies
Copy-Item "$assembliesDir\BlightedAlert.dll" "$deployDir\1.6\Assemblies\" -Force
Copy-Item "$assembliesDir\FarUtils.dll" "$deployDir\1.6\Assemblies\" -Force

Write-Host "Blighted Alert mod deployed successfully!" -ForegroundColor Green
