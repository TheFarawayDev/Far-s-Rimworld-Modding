...
# 2. Setup folders
$modDir = "c:\Users\meast\Downloads\Development\GAMES\rimworldModding\FarUtils"
...
if (-not (Test-Path $harmonyTarget)) {
    ...
    if (Test-Path $gcHarmony) {
        ...
        Write-Host "Copied Harmony DLL from Garbage Collector mod." -ForegroundColor Green
    } else {
        ...
        if (Test-Path $knownPath) {
            ...
            if ($found) {
                ...
                Write-Host "Could not find a local 0Harmony.dll." -ForegroundColor Red
                return
            }
        }
    }
...
# 4. Compile FarUtils.dll
Write-Host "Compiling FarUtils..." -ForegroundColor Green
...
$outputDll = "$assembliesDir\FarUtils.dll"
...
Write-Host "Successfully compiled FarUtils.dll!" -ForegroundColor Green

# 5. Deploy mod to RimWorld Mods folder
$deployDir = "$GamePath\Mods\FarUtils"
...
Write-Host "Far-utils Framework mod deployed successfully!" -ForegroundColor Green
