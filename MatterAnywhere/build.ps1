...
$modName = "MatterAnywhere"
$csprojPath = Join-Path $PSScriptRoot "Source\MatterAnywhere\MatterAnywhere.csproj"
...
if (Test-Path $aboutXmlPath) {
    ...
    if ($null -ne $xml.ModMetaData.supportedVersions) {
        foreach ($ver in $xml.ModMetaData.supportedVersions.li) {
            ...
            $sourceAssemblies = Join-Path $PSScriptRoot "Assemblies\*"
            if (Test-Path (Join-Path $PSScriptRoot "Assemblies")) {
